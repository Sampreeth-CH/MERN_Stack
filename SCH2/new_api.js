...
//     {id:2,task: "Study mongodb", completed: true}
// ]
...
//         req.on('end',()=>{
//             try{
//                 const urlSegments = path.split('/').filter(
//                     segment => segment.length > 0
//                 );
//                 const resourceId = urlSegments[2] ? parseInt(urlSegements[2]) : null;
//                 let data = (method ==='POST' || method === 'PUT')? JSON.parse(
//                     body || '{}'):{};
...
//                                 return;
//                             }
//                             const newTodo = (id: nextid++,task: data.task, completed)
//                             todos.push(newTodo);
//                             res.statusCode=201;
...
//                             if(resourceId)
//                             {
//                                 const index = tools.findIndex(t => t.id ===resourceId);
//                                 if(index !==-1)
//                                 {
//                                     todos[index]={...todos[index],task:data.task}
//                                 }
//                             }
//                     }
  ...
  if (path === API_PATH || path.startsWith(API_PATH + '/')) {
    ...
    req.on('end', () => {
      try {
        ...
        switch (method) {
          case 'GET':
            // R - READ: GET /api/todos or GET /api/todos/:id
            if (resourceId) {
              const todo = todos.find(t => t.id === resourceId);
              if (todo) {
                res.statusCode = 200;
          ...
          case 'PUT':
            // U - UPDATE: PUT /api/todos/:id
            if (resourceId) {
              const index = todos.findIndex(t => t.id === resourceId);
              if (index !== -1) {
                todos[index] = {
                  ...todos[index],
                  task: data.task || todos[index].task,
                  completed: data.completed !== undefined ? data.completed : todos[index].completed
                };
                res.statusCode = 200;
...
server.listen(PORT, () => {
  ...
  console.log(`Endpoints: ${API_PATH} and ${API_PATH}/:id`);
